"""
Evaluate VICReg encoder + residual transition + HER reward planner.
"""
import argparse
import json
import sys
...
from planning.vicreg_planner import load_vicreg_planner
...
def main():
    ...
    p.add_argument('--encoder', type=str, default='checkpoints/encoder_vicreg.pt')
    p.add_argument('--transition', type=str, default='checkpoints/transition_vicreg.pt')
    p.add_argument('--her-predictor', type=str, default='checkpoints/reward_predictor_her_vicreg.pt')
    ...
    p.add_argument('--out', type=str, default='evaluation/vicreg_planner_results.json')
    args = p.parse_args()

    print(f"VICReg planner on {args.env}")
    ...
    print("\n[2/2] VICReg planner...")
    planner, device = load_vicreg_planner(
    ...
    print(f"  VICReg:  reward={out['vicreg_planner']['reward_mean']:.3f}, success={out['vicreg_planner']['success_rate']:.2%}")
