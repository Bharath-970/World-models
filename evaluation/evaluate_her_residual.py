"""
Evaluate HER + residual transition planner vs random.
"""
import argparse
import json
import sys
...
from planning.her_residual_planner import load_her_residual_planner
...
def main():
    ...
    p.add_argument('--transition', type=str, default='checkpoints/transition_residual.pt')
    ...
    p.add_argument('--out', type=str, default='evaluation/her_residual_results.json')
    args = p.parse_args()

    print(f"Device: {args.device or 'auto'}")
    print(f"Evaluating HER+residual planner on {args.env}")
    ...
    print("\n[2/2] HER+residual planner...")
    planner, device = load_her_residual_planner(
    ...
    print(f"  HER+R:   reward={out['her_residual_planner']['reward_mean']:.3f} ± {out['her_residual_planner']['reward_std']:.3f}, success={out['her_residual_planner']['success_rate']:.2%}")
