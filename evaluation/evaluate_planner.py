"""
Evaluate CEM planner against random and PPO baselines.
"""
...
from planning.cem_planner import load_planner
...
def evaluate_planner(env_name, planner, num_episodes, max_steps=None, device='cpu'):
    env = gym.make(env_name)
    rewards = []
    successes = []
    steps_to_goal = []
    for _ in tqdm(range(num_episodes), desc='planner'):
        ...
        for t in range(max_steps or 10000):
            obs_img = obs['image']
            a = planner.plan(obs_img, num_steps=1)[0]
            obs, r, term, trunc, info = env.step(a)
            ep_r += r
            steps += 1
            if term and r > 0:
...
def main():
    ...
    print(f"Loading planner from {args.checkpoints} ...")
    planner, device = load_planner(
    ...
    print("\n[2/3] CEM planner...")
    plan_r, plan_s, plan_steps = evaluate_planner(args.env, planner, args.num_episodes, args.max_steps, device)
