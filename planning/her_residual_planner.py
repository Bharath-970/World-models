"""
HER + residual transition planner.

Combines:
- ResidualTransitionModel (z_{t+1} = z_t + scale * Δ, no collapse)
- GoalConditionedRewardPredictor (HER-trained, dense reward)
- Vectorized CEM in latent space
"""
import torch
import numpy as np
...
def load_her_residual_planner(
    encoder_path='checkpoints/world_model_best.pt',
    transition_path='checkpoints/transition_residual.pt',
    her_predictor_path='checkpoints/reward_predictor_her.pt',
    ...
    wm = torch.load(encoder_path, map_location=device)
    encoder.load_state_dict(wm['encoder_state_dict'])

    trans = torch.load(transition_path, map_location=device)
    transition.load_state_dict(trans['transition_state_dict'])

    rp = torch.load(her_predictor_path, map_location=device)
